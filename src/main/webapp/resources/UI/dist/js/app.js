  ...
  $.AdminLTE.layout = {
    ...
    fix: function () {
      ...
      if ($("body").hasClass("fixed")) {
        ...
        var controlSearchSideBar = $($.AdminLTE.options.controlSearchSideBarOptions.selector);
        if (typeof controlSearchSideBar !== "undefined") {
          if (controlSearchSideBar.height() > postSetWidth)
  ...
  $.AdminLTE.controlSidebar = {
    ...
    activate: function () {
      ...
      SearchSideBarBtn.on('click', function (e) {
          e.preventDefault();
          //If the sidebar is not open
          if (!SearchSideBar.hasClass('control-sidebar-open')
                  && !$('body').hasClass('control-sidebar-open')) {
            //Open the sidebar
      ...
      if ($('body').hasClass('fixed')) {
        ...
        _this._fixForFixed(SearchSideBar);
      } else {
        //If the content height is less than the sidebar's height, force max height
        if ($('.content-wrapper, .right-side').height() < sidebar.height()) {
          _this._fixForContent(sidebar);
          _this._fixForContent(SearchSideBar);
        }
      }
    },
    //Open the control sidebar
    open: function (generalbar, slide) {
      //Slide over content
      if (slide) {
    	  generalbar.addClass('control-sidebar-open');
      } else {
    ...
    close: function (generalbar, slide) {
      if (slide) {
    	  generalbar.removeClass('control-sidebar-open');
      } else {
        $('body').removeClass('control-sidebar-open');
      }
    },
    _fix: function (generalbar) {
      var _this = this;
      if ($("body").hasClass('layout-boxed')) {
    	  generalbar.css('position', 'absolute');
    	  generalbar.height($(".wrapper").height());
        $(window).resize(function () {
          _this._fix(generalbar);
        });
      } else {
    	  generalbar.css({
          'position': 'fixed',
    ...
    _fixForFixed: function (generalbar) {
    	generalbar.css({
        'position': 'fixed',
        'max-height': '100%',
        'overflow': 'auto',
    ...
    _fixForContent: function (generalbar) {
      $(".content-wrapper, .right-side").css('min-height', generalbar.height());
    }
  };
